...
# -*- coding: utf-8 -*-
"""
Driver for the Keithley 196 digital multimeter
"""
...
class Keithley196(Multimeter):

    """
    The Keithley 196 is a 5 1/2 digit auto-ranging digital multimeter. You can
    find the full specifications list in the `Keithley 196 user's guide`_.

    Example usage:

    >>> import instruments as ik
    >>> import quantities as pq
    >>> dmm = ik.keithley.Keithley196.open_gpibusb('/dev/ttyUSB0', 12)
    >>> print dmm.measure(dmm.Mode.resistance)

    .. _Keithley 196 user's guide: http://www.keithley.com/data?asset=803
    """

    def __init__(self, filelike):
        super(Keithley196, self).__init__(filelike)
    ...
    class Mode(IntEnum):
        """
        Enum containing valid measurement modes for the Keithley 196
        """
        voltage_dc = 0
        voltage_ac = 1
        resistance = 2
    ...
    class TriggerMode(IntEnum):
        """
        Enum containing valid trigger modes for the Keithley 196
        """
        talk_continuous = 0
        talk_one_shot = 1
        get_continuous = 2
    ...
    class ValidRange(Enum):
        """
        Enum containing valid range settings for the Keithley 196
        """
    ...
    def mode(self, newval):
        ...
        if not isinstance(newval, Keithley196.Mode):
    ...
    def trigger_mode(self, newval):
        if isinstance(newval, str):
            newval = Keithley196.TriggerMode[newval]
        if not isinstance(newval, Keithley196.TriggerMode):
            raise TypeError('Drive must be specified as a '
                            'Keithley196.TriggerMode, got {} '
    ...
    def input_range(self):
        ...
        if index == 0:
            ...
            mode = self.mode
            value = Keithley196.ValidRange[mode.name].value[index - 1]
            units = UNITS2[mode]
            return value * units
    ...
    def input_range(self, newval):
        ...
        mode = self.mode
        valid = Keithley196.ValidRange[mode.name].value
    ...
    def parse_status_word(statusword):  # pylint: disable=too-many-locals
        ...
        return {'trigger': Keithley196.TriggerMode(int(trigger)),
                'mode': Keithley196.Mode(int(function)),
...
UNITS2 = {
    Keithley196.Mode.voltage_dc:  pq.volt,
    Keithley196.Mode.voltage_ac:  pq.volt,
    Keithley196.Mode.current_dc:  pq.amp,
    Keithley196.Mode.current_ac:  pq.amp,
    Keithley196.Mode.resistance:  pq.ohm,
}
