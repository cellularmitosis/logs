...
int8_t fan_pin = 3;
int8_t onboard_ntc_pin = A0;
int8_t exhaust_ntc_pin = A1;
...
void setup() {
  ...
  pinMode(exhaust_ntc_pin, INPUT);
  pinMode(onboard_ntc_pin, INPUT);
...
void loop() {
  ...
  for(uint16_t i=0; i < samples; i++) {
    accumulator += analogRead(A1);
  }
  float temp_c = accumulator / float(samples);
  temp_c = ((512.0 - temp_c) / 100) + 25.0;
